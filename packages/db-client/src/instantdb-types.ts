...
export interface SchemaAttr {
  ...
  linkConfig: {
    forward: {
      ...
      namespace: string;
      attr: string;
      nsMap: SchemaNamespaceMap | undefined;
    };
    reverse:
      | {
          id: string;
...
export type SearchFilterOp =
  ...
  | "$lt"
  | "$isNull";
export type SearchFilter = [string, SearchFilterOp, unknown];

export type { InstantIssue } from "@instantdb/core";
