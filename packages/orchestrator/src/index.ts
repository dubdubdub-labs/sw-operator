...
// Orchestrator wires components; errors are surfaced from underlying layers.
...
export function createOrchestrator({
  ...
  return {
    async bootAndPrepare(snapshotId, opts) {
      const vm = await provider.instances.boot(
        snapshotId,
      ...
      if (opts?.credentialProfiles && credentialsInstaller) {
        const installs = (opts.credentialProfiles as CredentialProfile[]).map(
          (profile) => credentialsInstaller(provider, instanceId, profile)
        );
      ...
      const caps: ProviderCapabilities | undefined = provider.capabilities;
      return { instanceId, capabilities: caps };
    },

    async startSession(instanceId, agent, input, pm) {
      const spec = (agent as Agent<unknown>).toProcessSpec(input);
