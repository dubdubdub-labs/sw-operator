...
export function createClaudeSyncCommand(iterationId: string): string {
  // Use bash -c pattern for shell commands with PM2
  return `pm2 start bash --name claude-sync --no-autorestart -- -c 'CLOUD_CODE_ITERATION_ID=${iterationId} doppler run -- claude-sync sync'`;
}
